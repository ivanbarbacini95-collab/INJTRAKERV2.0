...
let marketData = {}; // {symbol: {price, change24h, lastPrice}}
...
function drawChart(){
  ...
  chart=new Chart(ctx,{
    type:'line',
    data:{labels:chartData.map((_,i)=>i), datasets:[{data:chartData,borderColor:borderColor,borderWidth:2,tension:0.3,fill:true,backgroundColor:(borderColor==="#22c55e")?"rgba(34,197,94,0.1)":"rgba(239,68,68,0.1)",pointRadius:0}]},
 ...
 * MARKET TABLE
 ********************/
async function loadMarketInitial(){
  for(let s of marketSymbols){
    try{
      const resp = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${s.toLowerCase()}&vs_currencies=usd&include_24hr_change=true`);
    ...
    }catch(e){ console.error("Market init error",s,e); }
  }
  renderMarketTable();
}
function renderMarketTable(){
  ...
  for(let s of marketSymbols){
    ...
    const price = marketData[s]?.price || 0;
    const change = marketData[s]?.change24h || 0;
...
function loop(){
  ...
  for(let i=0;i<marketSymbols.length;i++){
    const s = marketSymbols[i];
    let lastPrice = marketData[s]?.lastPrice || 0;
    let priceEl = tbody.rows[i].cells[1].querySelector(".digit-inner");
    let newPrice = lastPrice + (Math.random()-0.5)*0.01; // mock small change
    marketData[s].lastPrice = newPrice;
    updateDigits(priceEl, s, newPrice);

    const change = ((newPrice - priceEl.innerText)/priceEl.innerText)*100 || 0;
    tbody.rows[i].cells[2].innerText = change.toFixed(2)+"%";

    if(newPrice>lastPrice) tbody.rows[i].classList.add("flash-up");
    else if(newPrice<lastPrice) tbody.rows[i].classList.add("flash-down");
    setTimeout(()=>{tbody.rows[i].classList.remove("flash-up","flash-down");},500);
  }
