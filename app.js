...
async function loadData() {
  ...
  try {
    ...
    apr = (i.inflation * (Number(p.pool.bonded_tokens) + Number(p.pool.not_bonded_tokens)) / Number(p.pool.bonded_tokens)) * 100;
  } catch (e) {
...
fetchHistory();
setInterval(fetchHistory, 900000); // aggiorna ogni 15min
